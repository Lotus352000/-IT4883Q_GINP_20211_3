...
class ProductController extends Controller {
    ...
    {
        $products = Product::select('id', 'slug', 'producer_id', 'name', 'image', 'sku_code', 'OS', 'rate', 'created_at')
    ...
    public function save(Request $request)
    {
        ...
        $product->slug         = !empty($request->slug) ? $request->slug : str_slug($request->name);
    ...
    {
        $producers = Producer::select('id', 'name')->orderBy('name', 'asc')->get();
        $product   = Product::select('id', 'slug', 'producer_id', 'name', 'image', 'sku_code', 'monitor', 'front_camera', 'rear_camera', 'CPU', 'GPU', 'RAM', 'ROM', 'OS', 'pin', 'information_details', 'product_introduction')
            ->whereHas('product_details', function (Builder $query) {
                $query->where('import_quantity', '>', 0);
            })->where('id', $id)->with([
