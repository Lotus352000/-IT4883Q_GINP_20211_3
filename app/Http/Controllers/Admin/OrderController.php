...
class OrderController extends Controller {
    ...
    {
        $orders = Order::select('id', 'payment_method_id', 'order_code', 'status', 'name', 'email', 'phone', 'created_at')
            // ->where('status', '<>', 0)
            ->with([
                'user'           => function ($query) {
    ...
    {
        $order = Order::select('id', 'payment_method_id', 'order_code', 'name', 'email', 'phone', 'address', 'created_at')
            ->where([['id', $id]])->with([
                'user'           => function ($query) {
