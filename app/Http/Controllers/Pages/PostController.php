...
class PostController extends Controller {
    public function index()
    {
        ...
        $posts = Post::select('id', 'title', 'image', 'slug', 'created_at')->latest()->paginate(11);
    ...
    public function show(Request $request)
    {
        ...
        $post = Post::select('id', 'title', 'image', 'slug', 'content', 'created_at')
            ->where('slug', $request->slug)->first();

        if (!$post)
            abort(404);

        $suggest_posts = Post::select('id', 'title', 'image', 'slug', 'created_at')
