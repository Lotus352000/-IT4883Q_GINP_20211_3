...
class HomePage extends Controller {
    ...
    public function __invoke()
    {
        ...
        $products = Product::select('id', 'slug', 'name', 'image', 'monitor', 'front_camera', 'rear_camera', 'CPU', 'GPU', 'RAM', 'ROM', 'OS', 'pin', 'rate')
        ...
        $favorite_products = Product::select('id', 'slug', 'name', 'image', 'monitor', 'front_camera', 'rear_camera', 'CPU', 'GPU', 'RAM', 'ROM', 'OS', 'pin', 'rate')
            ->whereHas('product_detail', function (Builder $query) {
                $query->where('quantity', '>', 0);
            })
            ->with([
        ...
        $posts = Post::select('id', 'title', 'image', 'slug', 'created_at')->latest()->limit(4)->get();
