...
class ProductsController extends Controller {
    public function index(Request $request)
    {
        ...
        if ($request->has('price') && $request->input('price') != null) {
            ...
            })->select('id', 'slug', 'name', 'image', 'monitor', 'front_camera', 'rear_camera', 'CPU', 'GPU', 'RAM', 'ROM', 'OS', 'pin', 'rate')->orderBy('min_sale_price', $request->input('price'));
        } else {
            $query_products->select('id', 'slug', 'name', 'image', 'monitor', 'front_camera', 'rear_camera', 'CPU', 'GPU', 'RAM', 'ROM', 'OS', 'pin', 'rate')->latest();
        }
    ...
    public function getProduct(Request $request, $slug)
    {
        ...
        $product = Product::select('id', 'slug', 'producer_id', 'name', 'sku_code', 'monitor', 'front_camera', 'rear_camera', 'CPU', 'GPU', 'RAM', 'ROM', 'OS', 'pin', 'rate', 'information_details', 'product_introduction')
        ...
        $product_details = ProductDetail::where([['product_id', $product->id], ['import_quantity', '>', 0]])->with([
        ...
        $suggest_products = Product::select('id', 'slug', 'name', 'image', 'rate')
            ->whereHas('product_detail', function (Builder $query) {
                $query->where('quantity', '>', 0);
            })
            ->where([['producer_id', $product->producer_id], ['id', '<>', $product->id]])
            ->with([
                'product_detail' => function ($query) {
                    $query->select('id', 'product_id', 'quantity', 'sale_price', 'promotion_price', 'promotion_start_date', 'promotion_end_date')->where('quantity', '>', 0)->orderBy('sale_price', 'ASC');
                }
        ...
        })->where('product_id', $product->id)->with([
            'user' => function ($query) {
    ...
    public function addVote(Request $request)
    {
        ...
        $product       = Product::where('id', $request->product_id)->first();$product->rate = $rate;
        $product->save();
