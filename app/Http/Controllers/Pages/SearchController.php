...
class SearchController extends Controller {
    ...
    public function __invoke(Request $request)
    {
        if ($request->has('search_key') && $request->search_key != null) {
            ...
            $product_details = ProductDetail::select('id', 'product_id', 'quantity', 'sale_price', 'promotion_price', 'promotion_start_date', 'promotion_end_date')
                ->where('sale_price', '<', '%' . $request->search_key . '%')
            ...
            return view('pages.search')->with(['data' => ['advertises' => $advertises, 'posts' => $posts, 'products' => $products, 'product_details'=>$product_details]]);
        } else {
